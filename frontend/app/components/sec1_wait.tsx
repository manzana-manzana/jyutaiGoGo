...
type Ami = {
    ...
    startRecording: () => void;
    stopRecording: (apiAddress: string) => Promise<{ isResYes: boolean, isResNo: boolean, text: any ,isMatch:boolean} | undefined>;
};
...
const amiVoice: Ami = {
    ...
    stopRecording: async function(apiAddress){
        ...
        if (this.recording) {
            ...
            const isResYes = !!(resA.includes("はい") ||
                resA.includes("ＯＫ") ||
                resA.includes("いえす"))

            const isResNo = !!(resA.includes("いいえ"))

            const isMatch = !!(resA.includes("はい") ||
                resA.includes("ＯＫ") ||
                resA.includes("いえす") ||
                resA.includes("いいえ"));

            return {isResYes: isResYes, isResNo: isResNo, text: resA, isMatch:isMatch}
        }
    }
}
...
export default function Sec1_Wait()  {
    ...
    useEffect(() => {
        console.log(`[anime]soundFin:${isSoundFinish},recordingSuc:${isRecordingSuccessful},recoding:${isRecording}`)
    ...
    const getAmiVoice = async()=>{
        ...
        setTimeout(async() => {
            ...
            amiRes.isResNo && amiRes.isMatch? setIsNotTalk(true) : setIsNotTalk(false)
        ...
        <View style={styles.container}>
            ...
            <View style={[thisStyles.jamArea, { opacity: isJam ? 0.9:0 }]}>
                ...
                <View style={thisStyles.jamArea3}>
                    <Text style={[thisStyles.jamText3,{top:'50%',opacity: isRetry ? 1:0}]}>
                        もう一度話してください
                    </Text>
                </View>
                <Text style={[thisStyles.overlayText,
                    {top:'80%', fontSize:moderateScale(24), color:'#00B4AA',  opacity: isNotTalk ? 1:0}]}>
                    待機画面に戻ります
                </Text>
            </View>

            <View style={[thisStyles.micArea,{opacity: isSoundFinish  && !isRecordingSuccessful ? 1:0}]}>
                <Image style={{ width:horizontalScale(27)}}
                       resizeMode='contain'
